/**
 * GOOGLE APPS SCRIPT - WITH COMPLETE CORS FIX (GET + POST + OPTIONS)
 * Updated: 2025-11-15 - Fixed CORS for GET requests too
 *
 * CRITICAL FIX: This version handles CORS for ALL request types:
 * - OPTIONS (preflight)
...
function doOptions(e) {
  ...
  // Return CORS headers for preflight - use wildcard for simplicity
 ...
 * FIXED: Now includes proper CORS headers for all responses
 */
function doGet(e) {
  ...
  try {
    ...
    // CRITICAL FIX: Add CORS headers to ALL GET responses
    output.setHeader('Access-Control-Allow-Origin', '*');
    output.setHeader('Access-Control-Allow-Methods', 'POST, GET, OPTIONS');
...
function createSuccessResponse(data) {
  ...
  // Add CORS headers - use wildcard for maximum compatibility
  output.setHeader('Access-Control-Allow-Origin', '*');
  output.setHeader('Access-Control-Allow-Methods', 'POST, GET, OPTIONS');
  output.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization');
